...
class Keypad(Gtk.Box):
    def __init__(self, screen, ok_cb, cancel_cb, entry_max=5, error_msg=_("Invalid Number")):
        ...
        close_btn = self._gtk.Button("cancel", scale=0.66)
        ...
        self.ok_btn.set_size_request(
            -1, self._gtk.font_size * (4 if self.screen.vertical_mode else 5)
        )
    ...
    def _build_numpad(self):
        ...
        for i, (label, css_class) in enumerate(keys):
            if label == "B":
                btn = self._gtk.Button("backspace", scale=0.66, style="numpad_key")
            else:
    ...
    def add_extra_button(self, callback, icon=None, label=""):
        ...
        btn = self._gtk.Button(
            icon, label, "color2", self._gtk.bsidescale, Gtk.PositionType.LEFT, 1
        )
