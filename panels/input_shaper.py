...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.status = Gtk.Label(
            hexpand=True, vexpand=False, halign=Gtk.Align.START, ellipsize=Pango.EllipsizeMode.END
        )
    ...
    def start_calibration(self, widget, method):
        ...
        if method == "x":
            self._screen._send_action(
                self.calibrate_btn, "printer.gcode.script", {"script": "SHAPER_CALIBRATE AXIS=X"}
            )
        if method == "y":
            self._screen._send_action(
                self.calibrate_btn, "printer.gcode.script", {"script": "SHAPER_CALIBRATE AXIS=Y"}
            )
        if method == "both":
            self._screen._send_action(
                self.calibrate_btn, "printer.gcode.script", {"script": "SHAPER_CALIBRATE"}
            )
