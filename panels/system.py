            ...
            if info['configured_type'] == 'git_repo':
                if not info['is_valid'] or info['is_dirty']:
                    ...
                    if info['version'] == info['remote_version']:
                        return
                    label.set_markup("<b>" + _("Outdated by %d commits:") + "</b>\n" % len(info['commits_behind']))
                    grid.attach(label, 0, i, 1, 1)
                    i = i + 1
                    date = ""
                ...
                label.set_markup("<b>" + _("%s will be updated to version") + ": %s</b>" % (
                                program.capitalize(), info['remote_version']))
                grid.attach(label, 0, i, 1, 1)
                i = i + 1
        if "package_count" in info:
            label.set_markup("<b>" + _("%d Packages will be updated") + ":</b>\n" % info['package_count'])
